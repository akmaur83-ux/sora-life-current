...
test('G2 pruning only runs against the REAL catalogue, never the bundled seed', () => {
  ...
  // A fashion or grocery line is never judged against the wellness catalogue
  // at all — each has its own reconciliation against its own catalogue.
  assert.match(store, /\.filter\(\(l\) => !isFashionLine\(l\) && !isGroceryLine\(l\) && !productById\[l\.id\]\)/);
});
