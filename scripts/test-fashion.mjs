...
await test('the product card: badge, heart, quick-add, price with MRP struck, name, rating with count, swatches with +N', () => {
  ...
  assert.match(card, /<a class="fs-quick" aria-label="Choose size and colour for Meadow Linen Shirt — Sage" href="\/fashion\/p\/meadow-linen-shirt-sage">/);
...
await test('/fashion/search and the PDP stub: the stub shows the size × colour matrix from the variants', () => {
  ...
  const rowM = p.slice(p.indexOf('<th scope="row">M</th>'), p.indexOf('<th scope="row">L</th>'));
  assert.match(rowM, /<td data-stock="out">Out of stock<\/td><td data-stock="in">In stock<\/td>/, 'M/Sage out, M/Navy in, in that column order');
  assert.match(p, /Add to bag, arrive with the product page in the next phase/);
  assert.doesNotMatch(p, /<button[^>]*>Add to (bag|cart)/i, 'no add-to-bag that cannot check out');
...
await test('routes and the untouchables: /fashion, c/:slug, p/:slug are wired; checkout, cart mutation, Razorpay, auth, coupons are not in the diff', () => {
  ...
  for (const r of ['index element={<FashionHome />}', 'path="c/:slug" element={<FashionCategory />}', 'path="p/:slug" element={<FashionProductStub />}']) assert.ok(appSrc.includes(r), r);
  ...
  const untouchable = /^(src\/pages\/(Checkout|Cart|Product|Category|Shop)\.jsx|src\/lib\/(store\.jsx|cartLine\.js|cartQuote\.js|payments\.js|customerAuth\.jsx|adminAuth\.jsx|coupon[A-Za-z]*\.js|creator[A-Za-z]*\.js)|src\/components\/(Header|CategorySpotlight|ProductCard)\.jsx|src\/components\/category\/|api\/|scripts\/(ingest|deactivate|gallery))/;
