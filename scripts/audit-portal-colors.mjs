// ============================================================
// PORTAL COLOUR AUDIT — is purple really gone, and is the dark room dark?
//
...
const DIR = resolve(ROOT, 'reports/creator-portal-dark');
...
const AUDIT = `(() => {
  ...
  const hasGlass = [...document.querySelectorAll('.crp__panel, .ck-band, .ctier, .ck-share')].some((el) => (getComputedStyle(el).backdropFilter || getComputedStyle(el).webkitBackdropFilter || 'none') !== 'none');
  const storefrontLeak = document.querySelector('.hm-leaderboard') && !document.querySelector('.crp') ? getComputedStyle(document.body).getPropertyValue('--d-bg').trim() : '';
