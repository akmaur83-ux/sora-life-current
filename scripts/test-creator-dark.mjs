...
await test('brand green and gold carry the accents; rank colours sit beside the brand tokens', () => {
  ...
  assert.match(dark, /--d-bg: #0B1510/, 'near-black forest ground');
...
await test('frosted glass is on the structural panels only, and only in the portal', () => {
  const glass = /\.crp\.crp--dark \.crp__panel, [^{]*\.ctier-board[^{]*\{[^}]*backdrop-filter: blur\(16px\)/.exec(dark);
  assert.ok(glass, 'glass rule missing');
  const glassSelectors = glass[0].split('{')[0].split(',').map((s) => s.trim());
...
const portalFor = (tab) => component('src/pages/CreatorPortal.jsx', 'CreatorPortal', {
  ...
  money2, CreatorEarnings, CreatorHowItWorks, ...UI, CreatorPayouts, ...T, rankSlot: tiers.rankSlot, CreatorTermsPanel, TermsUpdatedLine,
});
...
await test('figures count up: rendered with the final value, tagged for the client, never a zero on the server', () => {
  const dash = render('dashboard');
  assert.match(dash, /<span class="ck-count" data-count="12640.5">₹12,640\.50<\/span>/);
  assert.match(dash, /<span class="ck-count" data-count="1240">1240<\/span>/);
  assert.match(dash, /<span class="ck-count" data-count="148060">₹1,48,060\.00<\/span>/);
  const earn = render('earnings');
  assert.match(earn, /data-count="6548.1">₹6,548\.10</);
  const tier = render('tier');
  assert.match(tier, /data-count="138420">₹1,38,420\.00</);
  assert.match(tier, /data-count="19188.6">₹19,188\.60</);
});
