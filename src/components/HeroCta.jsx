...
function CtaImage({ src, className }) {
  ...
  return !failed && <DeferredImage src={src} className={className} alt="" aria-hidden="true" loading="eager" onError={() => setFailed(true)} />;
}
...
export default function HeroCta({ cta, appearance, placement = 'flow', artworkOnly = false, active = true, children }) {
  ...
  return <div className={`hero-cta hero-cta--${placement}`} style={heroCtaStyle(a)}
    ...
    <Link to={cta.to} className="v2-btn v2-btn--sm hero-cta__button" tabIndex={active ? undefined : -1}>
      {active && a.textureUrl && <CtaImage key={a.textureUrl} src={a.textureUrl} className="hero-cta__texture" />}
      {active && a.iconUrl && a.iconSide === 'left' && <CtaImage key={a.iconUrl} src={a.iconUrl} className="hero-cta__icon" />}
      <span className="hero-cta__label">{children || cta.label}</span>
      {active && a.iconUrl && a.iconSide === 'right' && <CtaImage key={a.iconUrl} src={a.iconUrl} className="hero-cta__icon" />}
    </Link>
  </div>;
}
